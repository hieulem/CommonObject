function [ res ] = backward2( layers, map, truesize )
...
if strcmp(layers{1}.type,'lrn')
    res=map;
    return;
end;

if strcmp(layers{1}.type,'conv') || strcmp(layers{1}.type,'pool')
    res = imresize(map,[size(map,1) *layers{1}.stride(1) ,size(map,2) * layers{1}.stride(2)], 'nearest');
...
if strcmp(layers{1}.type,'pool') 
    shiftx = truesize(1) - size(res,1);
    shifty = truesize(2) - size(res,2);
     res = padarray(res,[shiftx,shifty],'post');
