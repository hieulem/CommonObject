...
%          textread(sprintf(VOCopts.imgsetpath,VOCopts.trainset),'%s')];
%           tic
%   for i=1:length(gtids)
%       recs(i)=PASreadrecord(sprintf(VOCopts.annopath,gtids{i}));
%   end
% 
% save('VOC2012_trainval_info.mat','recs','gtids');
% load('VOC2012_trainval_info.mat');
% save('VOC2012opts.mat','VOCopts');
...
for i=1:length(gtids)
    ...
    for clsi =1:20
        ...
        truncated = [recs(i).objects(clsinds).truncated];
        % assign ground truth class to image
        if isempty(clsinds)
            gt=-1;          % no objects of class
        elseif any(~(diff|truncated))
...
for clsi =1:20
    cls = VOCopts.classes{clsi};
    path = fullfile('VOC2012','trainval','imlist',cls);
    mkdir(path);
    recs_class = recs(ind(clsi,:)>0);
    save(fullfile(path,'imlist.mat'),'recs_class');
end;
